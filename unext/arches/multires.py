...
class MultiResUnet(nn.Module):
    ...
    def __init__(self,
        ...
        self.in_filters1 = (int(32 * self.alpha * 0.167) +
        ...
        self.in_filters2 = (int(32 * 2 * self.alpha * 0.167) +
        ...
        self.in_filters3 = (int(32 * 4 * self.alpha * 0.167) +
        ...
        self.in_filters4 = (int(32 * 8 * self.alpha * 0.167) +
        ...
        self.in_filters5 = (int(32 * 16 * self.alpha * 0.167) +
                            int(32 * 16 * self.alpha * 0.333) +
                            int(32 * 16 * self.alpha * 0.5))

        # Decoder path
        self.upsample6 = nn.ConvTranspose2d(self.in_filters5,
                                            32 * 8,
                                            kernel_size=(2, 2),
                                            stride=(2, 2))
        self.concat_filters1 = 32 * 8 * 2
        self.multiresblock6 = Multiresblock(self.concat_filters1, 32 * 8)
        self.in_filters6 = (int(32 * 8 * self.alpha * 0.167) +
                            int(32 * 8 * self.alpha * 0.333) +
                            int(32 * 8 * self.alpha * 0.5))

        self.upsample7 = nn.ConvTranspose2d(self.in_filters6,
                                            32 * 4,
                                            kernel_size=(2, 2),
                                            stride=(2, 2))
        self.concat_filters2 = 32 * 4 * 2
        self.multiresblock7 = Multiresblock(self.concat_filters2, 32 * 4)
        self.in_filters7 = (int(32 * 4 * self.alpha * 0.167) +
                            int(32 * 4 * self.alpha * 0.333) +
                            int(32 * 4 * self.alpha * 0.5))

        self.upsample8 = nn.ConvTranspose2d(self.in_filters7,
                                            32 * 2,
                                            kernel_size=(2, 2),
                                            stride=(2, 2))
        self.concat_filters3 = 32 * 2 * 2
        self.multiresblock8 = Multiresblock(self.concat_filters3, 32 * 2)
        self.in_filters8 = (int(32 * 2 * self.alpha * 0.167) +
                            int(32 * 2 * self.alpha * 0.333) +
                            int(32 * 2 * self.alpha * 0.5))

        self.upsample9 = nn.ConvTranspose2d(self.in_filters8,
                                            32,
                                            kernel_size=(2, 2),
                                            stride=(2, 2))
        self.concat_filters4 = 32 * 2
        self.multiresblock9 = Multiresblock(self.concat_filters4, 32)
        self.in_filters9 = (int(32 * self.alpha * 0.167) +
                            int(32 * self.alpha * 0.333) +
                            int(32 * self.alpha * 0.5))

        self.conv_final = Conv2d_batchnorm(self.in_filters9,
                                           num_classes,
                                           kernel_size=(1, 1),
                                           activation='None')
...
class Multiresblock(nn.Module):
    ...
    def __init__(self, num_in_channels, num_filters, alpha=1.67):
        ...
        self.shortcut = Conv2d_batchnorm(num_in_channels,
                                         num_out_filters,
                                         kernel_size=(1, 1),
                                         activation='None')

        self.conv_3x3 = Conv2d_batchnorm(num_in_channels,
                                         filt_cnt_3x3,
                                         kernel_size=(3, 3),
                                         activation='relu')

        self.conv_5x5 = Conv2d_batchnorm(filt_cnt_3x3,
                                         filt_cnt_5x5,
                                         kernel_size=(3, 3),
                                         activation='relu')

        self.conv_7x7 = Conv2d_batchnorm(filt_cnt_5x5,
                                         filt_cnt_7x7,
                                         kernel_size=(3, 3),
                                         activation='relu')
