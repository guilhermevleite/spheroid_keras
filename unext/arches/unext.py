...
class UNext(nn.Module):
    ...
    def __init__(self,
        ...
        dpr = [x.item() for x
               in torch.linspace(0, drop_path_rate, sum(depths))]
    ...
    def forward(self, x):
        ...
        out = F.relu(F.interpolate(
            self.dbn3(self.decoder3(out)),
            scale_factor=(2, 2),
            mode='bilinear'))

        out = torch.add(out, t2)
        out = F.relu(F.interpolate(
            self.dbn4(self.decoder4(out)),
            scale_factor=(2, 2),
            mode='bilinear'))
...
class shiftmlp(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x_shift = [torch.roll(x_c, shift, 2) for
                   x_c, shift in
        ...
        xn = F.pad(xn, (self.pad, self.pad, self.pad, self.pad), "constant", 0)
        xs = torch.chunk(xn, self.shift_size, 1)
        x_shift = [torch.roll(x_c, shift, 3) for
                   x_c, shift in
                   zip(xs, range(-self.pad, self.pad+1))]
