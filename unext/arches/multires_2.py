...
class MultiResUnet(nn.Module):
    def __init__(self,
        ...
        self.multiresblock6 = Multiresblock(
                input_features=self.concat_filters1,
                corresponding_unet_filters=self.filters*8)

        self.in_filters6 = (int(self.filters*8*self.alpha*0.5) +
                            int(self.filters*8*self.alpha*0.167) +
                            int(self.filters*8*self.alpha*0.333))

        self.upsample7 = nn.ConvTranspose2d(in_channels=self.in_filters6,
        ...
        self.multiresblock7 = Multiresblock(
                input_features=self.concat_filters2,
                corresponding_unet_filters=self.filters*4)

        self.in_filters7 = (int(self.filters*4*self.alpha*0.5) +
                            int(self.filters*4*self.alpha*0.167) +
                            int(self.filters*4*self.alpha*0.333))

        self.upsample8 = nn.ConvTranspose2d(in_channels=self.in_filters7,
        ...
        self.in_filters8 = (int(self.filters * 2 * self.alpha * 0.5) +
                            int(self.filters * 2 * self.alpha * 0.167) +
                            int(self.filters*2*self.alpha*0.333))

        self.upsample9 = nn.ConvTranspose2d(in_channels=self.in_filters8,
        ...
        self.in_filters9 = (int(self.filters*self.alpha*0.5) +
                            int(self.filters*self.alpha*0.167) +
                            int(self.filters*self.alpha*0.333))

        self.conv_final = Conv2d_batchnorm(input_features=self.in_filters9,
                                           num_of_filters=self.nclasses,
                                           kernel_size=(1, 1),
                                           activation='None')
    ...
    def __init__(self,
        ...
        """
            ...
            Arguments:
              ...
              alpha - 1.67 - factor used in the paper to dervie number of
              filters for multiresunet filters from Unet filters
            Returns - None
        """
        super().__init__()
        ...
        self.conv2d_bn_1x1 = Conv2d_batchnorm(
                input_features=input_features,
                num_of_filters=(int(self.W*0.167) +
                                int(self.W*0.333) +
                                int(self.W*0.5)),
                kernel_size=(1, 1),
                activation='None',
        ...
        self.conv2d_bn_3x3 = Conv2d_batchnorm(
                input_features=input_features,
                num_of_filters=int(self.W*0.167),
                kernel_size=(3, 3),
                activation='relu',
        ...
        self.conv2d_bn_5x5 = Conv2d_batchnorm(
                input_features=int(self.W*0.167),
                num_of_filters=int(self.W*0.333),
                kernel_size=(3, 3),
                activation='relu',
                padding=1)

        self.conv2d_bn_7x7 = Conv2d_batchnorm(
                input_features=int(self.W*0.333),
                num_of_filters=int(self.W*0.5),
                kernel_size=(3, 3),
                activation='relu',
                padding=1)

        self.batch_norm1 = nn.BatchNorm2d(int(self.W*0.5) +
                                          int(self.W*0.167) +
                                          int(self.W*0.333), affine=False)
...
class Respath(nn.Module):
    def __init__(self,
        ...
        self.conv2d_bn_1x1_common = Conv2d_batchnorm(
                input_features=self.filters,
                num_of_filters=self.filters,
                kernel_size=(1, 1),
                activation='None',
                padding=0)

        self.conv2d_bn_3x3_common = Conv2d_batchnorm(
                input_features=self.filters,
                num_of_filters=self.filters,
                kernel_size=(3, 3),
                activation='relu',
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        if self.respath_length > 1:
            for _ in range(self.respath_length):
...
class Conv2d_batchnorm(nn.Module):
    def __init__(self,
        ...
        # TODO Este conv2 está recebendo argumentos inválidos
        self.conv1 = nn.Conv2d(in_channels=input_features,
