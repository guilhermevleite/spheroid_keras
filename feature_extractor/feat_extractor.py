...
# TODO : Send PATHs via arguments or config file
...
def main():
    ...
    # TODO : All of the data has to be extracted, not only 97%
    breakpoint()
    hi_hello, val_img_ids = train_test_split(img_ids, test_size=0.97)
    ...
    # TODO : This information does not matter
    ...
    # count = 0
    # TODO : Save extracted feature in the same folder as the model
    for c in range(config['num_classes']):
        os.makedirs(os.path.join(OUTPUTS_PATH, config['name'], str(c)), exist_ok=True)

    with torch.no_grad():
        # TODO : We don't need a target to extract features
        # TODO : What is meta?
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            # TODO : We need to send these to GPU, otherwise it will take forever
            # compute output
            output = model(input)

            # TODO : No metrics in feat extraction
            iou, dice = iou_score(output, target)
            iou_avg_meter.update(iou, input.size(0))
            dice_avg_meter.update(dice, input.size(0))

            # TODO : We want to save output, and not sigmoid it
            output = torch.sigmoid(output).cpu().numpy()
            output[output >= 0.5] = 1
            output[output < 0.5] = 0

            # TODO : No images to save here
            # TODO : Append extracted features to file
